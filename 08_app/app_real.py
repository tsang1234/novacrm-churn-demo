...
if page == "🏠 Dashboard":
    ...
    with c1:
        st.metric("Entreprises analysées", f"{int(stats.get('total_companies', 0)):,}")
    with c2:
        n_high = int(stats.get("high_risk", 0))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    n = len(df)
    mrr_total = df["mrr"].astype(float).sum()
    st.markdown(
    ...
    if filter_mrr > 0:
        filtered = filtered[filtered["mrr"].astype(float) >= filter_mrr]
    ...
    for _, row in filtered.iterrows():
        ...
        churn     = float(row.get("churn_probability", 0))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if min_prob > 0:
        filtered = filtered[filtered["churn_probability"].astype(float) * 100 >= min_prob]
    if filter_size:
    ...
    display["churn_probability"] = (display["churn_probability"].astype(float) * 100).round(1)
    display["mrr"] = display["mrr"].astype(float).round(0).astype(int)
    ...
    styled = display.style.applymap(color_tier, subset=["Tier"])
